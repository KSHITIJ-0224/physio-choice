    ...
    <section id="pricing" className="py-28 px-6 bg-surface">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16 max-w-2xl">
          <span className="text-[10px] uppercase tracking-[0.25em] font-bold text-primary">Plans</span>
        ...
        <div className="grid md:grid-cols-3 gap-px bg-border">
          {plans.map((p) => (
            <div
              key={p.name}
              className={`p-10 flex flex-col ${p.highlight ? "bg-foreground text-background" : "bg-surface"}`}
            >
              <div className="flex items-start justify-between">
                <h3 className="font-display text-xl font-bold uppercase tracking-tight">{p.name}</h3>
                {p.highlight && (
                  <span className="bg-primary text-primary-foreground text-[9px] font-black uppercase px-2 py-1 tracking-tight">
                    Most Chosen
                  </span>
                )}
              </div>
              <div className="mt-10 flex items-baseline gap-2">
                <span className="font-display text-6xl font-extrabold tracking-tighter">{p.price}</span>
                <span className={`text-xs uppercase tracking-widest ${p.highlight ? "text-background/60" : "text-muted-foreground"}`}>
                  {p.unit}
                </span>
              </div>
              <p className={`mt-4 text-sm leading-relaxed ${p.highlight ? "text-background/70" : "text-muted-foreground"}`}>
                {p.desc}
              </p>
              <ul className={`mt-8 space-y-3 text-sm ${p.highlight ? "text-background/90" : ""}`}>
                {p.features.map((f) => (
                  <li key={f} className="flex gap-3">
                    <span className={p.highlight ? "text-primary" : "text-primary"}>—</span>
                    {f}
                  </li>
                ))}
              </ul>
              <button
                className={`mt-10 py-4 text-[11px] font-bold uppercase tracking-[0.25em] transition-all ${
                  p.highlight
